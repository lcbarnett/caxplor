...
int sim_ana(int argc, char* argv[])
{
	...
	for (size_t b=0; b<=bmax; ++b) {
		...
		for (size_t s=0; s<S; ++s) {
			rt_randomb(B,rtab,b,&brng);
			H[b][s] = oom*rt_entro(B,rtab,m,iters);
		}
	...
	char* const gpfile = sm_printf(sm,"caana_B%d_m%d_i%d_S%zu",B,m,iters,S);
	FILE* const gp = gp_fopen(gpfile,NULL,NULL);
	fprintf(gp,"set title 'CA entropy vs bias (B = %d, m = %d, iters = %d, samples = %zu)'\n",B,m,iters,S);
