...
int sim_ana(int argc, char* argv[])
{
	...
	double*  const HH = calloc((bmax+1)*samps,sizeof(double));
	TEST_ALLOC(HH);
	...
	for (size_t b=0; b<=bmax; ++b) {
		...
		for (size_t s=0; s<samps; ++s) {
			...
			H[b][s] = oom*rt_entro(rsiz,rtab,m,iters,bin);
		}
	...
	char* const gpfile = sm_printf(sm,"caana_B%d_m%d_i%d_S%zu",rsiz,m,iters,samps);
